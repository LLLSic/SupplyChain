...
contract SupplyChain {
    ...
    //zheligail!!!!!
    function AddContract(address receiver, address prover, uint amount) {
        //核心机构入链
        companysInChain.push(Company(msg.sender, 0));
        companys[msg.sender] = companysInChain[0];
        //见证方（这里是银行）入链
        ...
        receipts[receipts.length - 1].from = msg.sender;
    ...
    function Transfer(address receiver, uint amount, uint receiptID) {
        //由票据拥有者发起
        require(
        ...
        receipts[receiptID].leftValue -= amount;
        emit NewReceipt("fresh", receipts[receiptID].receiptID, receipts[receiptID].from, receipts[receiptID].to, receipts[receiptID].mount, receipts[receiptID].prover, receipts[receiptID].financinged, receipts[receiptID].settled, receipts[receiptID].transferRecord, receipts[receiptID].leftValue);
    }
